...
class CountingConsumer:
    """
    CPU-optimized consumer with true batched inference.
    Maintains all V2.1 functionality while maximizing CPU utilization.
    """
    ...
    def __init__(self, config, visualize_mode=None):
        ...
        self.enable_parallel = self.consumer_config.get('enable_parallel', True)
        self.max_workers = self.consumer_config.get('max_workers', 4)
        self.executor = None
    ...
    def get_all_camera_data(self, force=False):
        """Get count data for all cameras."""
        all_camera_data = {}
        ...
        for camera_id in self.object_counters.keys():
            ...
            with self.counter_locks[camera_id]:
                ...
                all_camera_data[camera_id] = count_data
    ...
    def _process_camera_batch(self, camera_id, frames):
        """Process a batch of frames for a single camera (worker function)"""
        if camera_id not in self.object_counters:
            return
        ...
        for frame_data in frames:
            try:
                ...
                logger.error(f"Error processing frame for {camera_id}: {e}", exc_info=True)
        
        return camera_id, results
    ...
    def _process_batch(self):
        ...
        if self.enable_parallel and self.executor:
            ...
            for camera_id, frames in camera_frames.items():
                future = self.executor.submit(self._process_camera_batch, camera_id, frames)
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if self.visualize_mode and results:
                        for result in results:
                            self._handle_visualization(result, camera_id)
            ...
            for camera_id, frames in camera_frames.items():
                _, results = self._process_camera_batch(camera_id, frames)
        ...
        # logger.info(f"Processed batch of {len(self.pending_frames)} frames in {processing_time:.3f}s")
        
        self.pending_frames.clear()
        self.last_batch_time = time.time()
    
    def _handle_visualization(self, result, camera_id):
        """Handle visualization for a processed frame"""
